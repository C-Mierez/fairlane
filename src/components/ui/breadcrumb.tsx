...
>(({ asChild, className, ...props }, ref) => {
    ...
    return (
        <Comp
            ref={ref}
            prefetch={false}
            className={cn("hover:text-foreground underline transition-colors hover:font-medium", className)}
            {...props}
        />
    );
});
...
const BreadcrumbSeparator = ({ children, className, ...props }: ComponentProps<"li">) => (
    <li role="presentation" aria-hidden="true" className={cn("[&>svg]:size-5 md:[&>svg]:size-6", className)} {...props}>
        {children ?? <SlashIcon />}
    </li>
);
